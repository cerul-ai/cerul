...
## Embedded PaddlePaddle OCR assets

Copyright PaddlePaddle Authors.

Cerul embeds the PP-OCRv6 small detection and recognition ONNX weights and
recognition dictionary under the Apache License, Version 2.0. The weight files
are unmodified. Intermediate shape annotations are removed from the in-memory
graph before inference; no weight file is changed.

Sources:

- https://huggingface.co/PaddlePaddle/PP-OCRv6_small_det_onnx/tree/28fe5895c24fd108c19eb3e8479f4ab385fbfc62
- https://huggingface.co/PaddlePaddle/PP-OCRv6_small_rec_onnx/tree/b8f84f0b80c529de40b4fbb3544b84fa7233a513

The dictionary is extracted from the recognition model's inference.yml, with
the CTC space character appended. See models/README.md in the source repository
for file hashes and extraction details.
...
executables built from FFmpeg 7.1.4 with statically linked x264 and zlib.
Full media-tool license texts accompany binary archives in `licenses/` (source
checkout: `packaging/licenses/`). These executables
...
- zlib 1.3.1 source (zlib license): https://github.com/madler/zlib/releases/download/v1.3.1/zlib-1.3.1.tar.gz

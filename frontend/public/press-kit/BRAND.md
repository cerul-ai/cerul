# Cerul Brand Copy — Canonical Reference (MASTER)

> **This repo is the upstream master.** Every other repo's `BRAND.md` is a downstream
> mirror of this file — when copy changes, change it *here first*, then sync the mirrors.
...
## Downstream mirrors (keep in sync when this changes)

`cerul-web` · `cerul-cli` · `cerul-cloud` · `cerul-status` · `cerul-flow` · `cerul-planning` ·
`cerul-python` (README) · `cerul-app` (README). Each holds a copy of the two slogans + descriptions.

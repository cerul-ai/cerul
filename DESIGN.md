...
atomic overview publication and resumable checkpoints. JSON returns scene, chapter
and summary records with per-stream errors; exit 6 means partial analysis. Dry runs
publish nothing and make no model calls. Supplying `--prompt`, reference `--image`,
`--from/--to` or `--stream` selects question-specific analysis: up to 120 sampled
frames plus in-range cached text, a typed answer with sampled timestamp evidence,
and independent content-keyed results under `analysis/requests/`. Ranges use
half-open episode microseconds. Reference images are comparison inputs, never
video observations. True SSE generation supports Gemini and compatible OpenAI
Chat Completions endpoints. Human output streams decoded answer text; JSON mode
